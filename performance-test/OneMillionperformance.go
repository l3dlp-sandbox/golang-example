...
func PerformanceTestMain(){
	var err error
	logFile, err = os.OpenFile("/home/damitha/go/src/golang-example/performance-test/result_billing.txt",
	...
	for i:=0;i<=3;i++{
		...
		wg.Add(2)
		wg.Wait()
		f, err := os.OpenFile("/home/damitha/go/src/golang-example/performance-test/result_file.txt",
			os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
		if err != nil {
			fmt.Println(err)
		}
...
func logBillingAccount(ban string){
	m:=sync.Mutex{}
	m.Lock()
...
func loadAccounts(i int){
	...
	r,e:=os.Open(fmt.Sprintf("/home/damitha/go/src/golang-example/performance-test/1m/%v",i))
	if e !=nil{
		fmt.Println(r)
	}
...
func worker(id int,s chan bool,t chan []string, wg *sync.WaitGroup){
	for{
		...
		case t:= <-t:
			...
			txnId:= fmt.Sprintf("0000%v",orderRef)
			atomic.AddInt64(&orderRef,1)
			var a account
			a=getAccount()
			...
			res:=makePayment(t,txnId,txnId)
			if res {
				logBillingAccount(a.BillingAccountNumber)
				mySqlInsert(a.BillingAccountNumber, "1", "gmo", txnId)
			}
			s <- true
		}
	}
...
func makePayment( tokens []string, uuid string, txnId string)bool{
	url := PaymentUrl
	method := "POST"
	s:=fmt.Sprintf(`{
    "uuid": "%v",
...
func mySqlInsert(bAN,isDefault,gateway,orderRef string){
	m:=sync.Mutex{}
	m.Lock()
	stmt,err:=Db.Prepare("UPDATE v3_payment_instruments SET billing_account_number=?,is_default=?,gateway=?," +
		"updated_at=now() where order_ref = ?")
	if err !=nil{
		fmt.Println("Error in preparing insert statement:",err)
	}
	r,err:=stmt.Exec(bAN,isDefault,gateway,orderRef)
	if err !=nil{
		fmt.Println(err)
		return
	}
